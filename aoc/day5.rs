...
use std::{io::{Read, BufReader, BufRead}, collections::HashMap};
...
fn solve1<R: Read>(reader: R) -> Result<String> {
    ...
    for line in assignments {
        ...
        for col in (0..line.len()).step_by(4) {
            ...
            buckets.entry(index as u32).or_insert(Vec::new()).push(letter);
        }
    }
...
move 1 from 1 to 2";
    ...
    #[test]
    fn solve1_works() {
        assert_eq!(String::from("CMZ"), solve1(Cursor::new(TEST_DOCUMENT)).unwrap());
    }
